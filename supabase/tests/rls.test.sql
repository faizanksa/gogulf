...
begin
  ...
  --
  -- One more name is allow-listed here, deliberately, by 0015: a payer is never
  -- authenticated, so the invoice flow needs a door for them. It is narrow by
  ...
  --     Read-only.
  -- open_invoice_payment_request(text,text) WAS a second door in 0015 and is not
  -- any more (0017): it stores a caller-supplied provider order id, so with the
  -- public anon key anyone could plant an invented order on any issued invoice and
  -- jam it (found on staging in the first real TEST payment). It is service_role
  -- only, asserted below. Adding a name here must be as deliberate as adding one
  -- anywhere else in this list — it is not a way around the assertion, it is the
  -- assertion stating precisely which door exists and why.
  select string_agg(p.proname, ', ') into bad
    from pg_proc p join pg_namespace n on n.oid = p.pronamespace
   where n.nspname = 'public' and p.prosecdef
     and p.prorettype not in ('pg_catalog.trigger'::regtype, 'pg_catalog.event_trigger'::regtype)
     and p.proname not in ('public_invoice_view')
  ...
  -- The allow-list itself is asserted shut: exactly this one name, and it must
  -- still exist and still be SECURITY DEFINER — a migration that drops or weakens
  -- it without updating this file is caught here, not missed by the exclusion
  -- above quietly widening to cover its absence.
  perform test_assert(
    (select count(*) from pg_proc p join pg_namespace n on n.oid = p.pronamespace
      where n.nspname = 'public' and p.prosecdef
        and p.proname in ('public_invoice_view')
        and has_function_privilege('anon', p.oid, 'EXECUTE')) = 1,
    'exactly the one documented function is anon-executable SECURITY DEFINER — not fewer, not more');

  -- The order-recording function is the server's alone: not anon, not signed-in
  ...
  perform test_assert(
    ...
    'open_invoice_payment_request is executable by service_role alone (0017): no browser-reachable role can plant an order id');
  ...
  select string_agg(p.proname, ', ') into bad
   ...
   where n.nspname = 'public' and p.prosecdef
     ...
     -- public_invoice_view (0015) is the one payer-facing door documented in the anon
     -- allow-list above; it is granted to authenticated as well so staff can preview a

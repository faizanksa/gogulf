...
// While REGISTERED_ADDRESS is unpublished (see lib/legal.js), this is empty
// and no location/address is asserted anywhere in the JSON-LD.
export const ORG_OFFICES = REGISTERED_ADDRESS.lines.length
  ? [
      {
...
/** schema.org PostalAddress for the registered office, or null while unpublished. */
export function registeredPostalAddress() {
  if (!REGISTERED_ADDRESS.lines.length) return null;
  return {
    "@type": "PostalAddress",
    ...
    "@graph": [
      {
        ...
        // rendered in the footer and the legal pages. Omitted entirely while
        // LEGAL_ENTITY is unpublished (see lib/legal.js) rather than emitting
        // empty strings.
        ...(LEGAL_ENTITY.name ? { legalName: LEGAL_ENTITY.name } : {}),
        ...(LEGAL_ENTITY.gstin ? { taxID: LEGAL_ENTITY.gstin, vatID: LEGAL_ENTITY.gstin } : {}),
        ...
        ...(registeredPostalAddress() ? { address: registeredPostalAddress() } : {}),
        areaServed: Object.entries(GULF_COUNTRIES).map(([name, code]) => ({
          "@type": "Country",
          name,
          identifier: code,
        })),
        ...(ORG_OFFICES.length ? { location: ORG_OFFICES.map((office) => ({
          "@type": "Place",
          name: office.name,
          address: {
            "@type": "PostalAddress",

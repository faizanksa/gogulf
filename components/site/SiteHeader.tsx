...
/**
 * Server-rendered header, in two parts.
 *
 * The utility bar (desktop only) carries the company record where a careful visitor
 * looks first: the operating company, its CIN, and "Verify Go Gulf" — checkable before
 * attractive. The language menu lives there too. It scrolls away; the main bar sticks.
 *
 * Only SiteNav (current page + mobile menu) and, when more than one language is active,
 * LanguageMenu run in the browser; the call to action, the icons and every word are
 * rendered here and handed to them, so Button, the icon set and the catalogues stay
 * out of the JavaScript every page loads.
 *
 ...
 *
 * The brand name, tagline and legal name stay in English in every language.
 */
export async function SiteHeader({ switcher }: { switcher: SwitcherData | null }) {
  ...
  const verify = { href: href("/verify"), label: t("header.verify") };
      ...
      <div className={styles.utility}>
        <div className={styles.utilityInner}>
          ...
          <div className={styles.utilityActions}>
            <Link href={verify.href} className={styles.verify}>
      ...
      <header className={styles.header}>
        <div className={styles.inner}>
          <div className={styles.brand}>
            ...
            <span className={styles.tagline} lang={englishText}>
              {COMPANY.tagline}
            </span>
          </div>
          <SiteNav
            ...
            icons={{
              ...
              shield: <Icon name="shield" size={20} />,
            }}
